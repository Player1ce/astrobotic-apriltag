...
public:
  ...
  void processImage(cv::Mat& image, cv::Mat& image_gray) {
    ...
    cv::Mat cameraMatrix = (cv::Mat1d(3, 3) << 354.2127, 0, 471.3652, 0, 354.7361, 362.7087, 0, 0, 1);
    cv::Mat distortionCoefficients = (cv::Mat1d(1, 5) << 0.0135, -0.0306, -0.0002, 0.0002, 0.0052);
    // Undistort image here
    cv::undistort(image_gray, undistort_img, cameraMatrix, distortionCoefficients);

    vector<AprilTags::TagDetection> detections = m_tagDetector->extractTags(undistort_img);
    if (m_timing) {
      double dt = tic()-t0;
    ...
    if (m_draw) {
      for (int i=0; i<detections.size(); i++) {
        ...
        detections[i].draw(undistort_img);
      }
      imshow(windowName, undistort_img); // OpenCV call
    }
  }
